...
class CoHostController extends GetxController with StateMixin<List<Contact>> {
  ...
  importFromContacts() async {
    try {
      ...
      log("Import contacts :: $err");
      SnackBarHelper.show("$err");
    }
  }

  addCoHostLocally(Contact datum, [int? index]) {
    List<Contact> tempList = state ?? [];
  ...
  addCoHostApi(Contact datum) async {
    try {
      ...
      final result = await ApiCall.post(ApiRoutes.cohosts, body: body);
      actionLoading = false;
      final responseDatum = Contact.fromJson(
          (result.data is List) ? result.data[0] : result.data);
      addCoHostLocally(responseDatum);
  ...
  removeCoHost(Contact value) async {
    ...
    try {
      ...
      log("$err :: $s");
      addCoHostLocally(value, prevIndex);
      SnackBarHelper.show("$err");
    }
  }
